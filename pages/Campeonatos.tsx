          ...
          <div className="flex flex-col md:flex-row justify-between items-end gap-6">
            <div>
               <div className="flex items-center gap-3 mb-2">
                 ...
                 <span className="text-[10px] font-black uppercase text-yellow-400 tracking-[0.4em] oswald italic">KDO Official Season</span>
               </div>
               <h1 className="text-5xl font-black italic oswald uppercase text-white mb-4 tracking-tighter leading-none">Temporada <span className="text-yellow-400">2026</span></h1>
               <div className="flex gap-4">
                  <button onClick={() => setActiveTab('torneos')} className={`px-8 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all ${activeTab === 'torneos' ? 'bg-yellow-400 text-black shadow-xl shadow-yellow-400/20' : 'text-zinc-500 hover:text-white'}`}>Torneos Activos</button>
                  <button onClick={() => setActiveTab('posiciones')} className={`px-8 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all ${activeTab === 'posiciones' ? 'bg-yellow-400 text-black shadow-xl shadow-yellow-400/20' : 'text-zinc-500 hover:text-white'}`}>Tabla de Posiciones</button>
               </div>
            </div>
            {activeTab === 'posiciones' && (
              <div className="flex items-center gap-4 w-full md:w-auto">
                 <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="bg-zinc-900 border border-zinc-800 text-white text-[11px] font-black uppercase rounded-xl px-6 py-4 outline-none focus:border-yellow-400 transition-all flex-grow md:flex-none appearance-none cursor-pointer">
                    {categories.map(c => <option key={c} value={c}>{c}</option>)}
                 </select>
                 <button onClick={handleDownloadStandings} className="bg-white text-black hover:bg-yellow-400 p-4 rounded-xl transition-all shadow-xl"><Download size={20} /></button>
              </div>
            )}
          </div>
        </header>
        ...
        {activeTab === 'torneos' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            {championships.map((ch) => (
              <div key={ch.id} className="bg-zinc-900 border border-zinc-800 rounded-[2.5rem] overflow-hidden flex flex-col shadow-2xl hover:border-yellow-400/50 transition-all group relative">
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="p-10 flex-grow flex flex-col justify-center">
                    <div className="flex items-center gap-3 mb-6">
                      <span className={`text-[9px] font-black uppercase px-3 py-1 rounded-full border ${ch.status === 'En curso' ? 'bg-emerald-600/10 text-emerald-500 border-emerald-500/20' : 'bg-yellow-400/10 text-yellow-400 border-yellow-400/20'}`}>{ch.status}</span>
                    </div>
                    <h3 className="text-4xl font-black oswald uppercase text-white mb-6 group-hover:text-yellow-400 transition-colors">{ch.name}</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-zinc-500 text-[10px] font-black uppercase tracking-widest mb-8">
                      <div className="flex items-center gap-3"><Calendar size={18} className="text-yellow-400" /> {ch.dates}</div>
                      <div className="flex items-center gap-3"><MapPin size={18} className="text-yellow-400" /> {ch.tracks}</div>
                    </div>
                    <div className="flex gap-4">
                       <button 
                       ...
                       <button 
                        ...
                        className="bg-yellow-400 text-black px-8 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 transition-all shadow-lg hover:bg-yellow-500"
                       >
                ...
                {expandedChamp === ch.id && ch.events && (
                  <div className="border-t border-zinc-800 bg-black/40 p-10 animate-in slide-in-from-top-4 duration-300">
                    <h4 className="text-xl font-black oswald uppercase text-white mb-8 italic flex items-center gap-3">
                      <Calendar className="text-yellow-400" /> Cronograma de Fechas <span className="text-zinc-600">Season 2026</span>
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                      {ch.events.map((ev) => (
                        <div key={ev.id} className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl relative overflow-hidden group/ev hover:border-yellow-400 transition-all">
                           ...
                           <p className="text-white font-black oswald uppercase text-lg leading-tight mb-2 group-hover/ev:text-yellow-400 transition-colors">{ev.name}</p>
                           <p className="text-[10px] font-bold text-zinc-500 uppercase flex items-center gap-1.5 mb-4">
                              <Calendar size={12} className="text-yellow-400" /> {ev.date}
                           </p>
                           ...
                           <div className="flex items-center justify-between">
                              <span className={`text-[8px] font-black uppercase px-2 py-1 rounded border ${
                                ...
                                'bg-yellow-400/5 text-yellow-400/40 border-yellow-400/10'
                              }`}>
                                {ev.status}
                              </span>
          ...
          <div className="animate-in fade-in slide-in-from-right-2 duration-500">
            <div className="bg-zinc-900 border border-zinc-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="bg-black p-6 px-10 border-b border-zinc-800 flex justify-between items-center">
                 <div className="flex items-center gap-3">
                    <ListOrdered className="text-yellow-400" size={20} />
                       ...
                       {standingsPilots.map((p, i) => (
                          <tr key={p.id} className="hover:bg-yellow-400/[0.03] transition-colors group">
                             <td className="px-10 py-5 text-center">
                                <span className={`text-2xl font-black oswald italic ${i === 0 ? 'text-yellow-400' : i < 3 ? 'text-zinc-200' : 'text-zinc-700'}`}>0{i + 1}</span>
                             </td>
                             <td className="px-10 py-5 text-center">
                                <span className="bg-black border border-zinc-800 text-white font-black oswald px-3 py-1 rounded text-base group-hover:text-yellow-400 transition-colors">#{p.number}</span>
                             </td>
                             <td className="px-10 py-5">
                                <p className="text-xs font-black text-white uppercase group-hover:text-yellow-400 transition-all">{p.name}</p>
                                <p className="text-[8px] text-zinc-600 uppercase font-black">{p.association}</p>
                             </td>
                             <td className="px-10 py-5 text-center text-xs font-black text-white oswald group-hover:text-yellow-400 transition-colors">{p.stats?.wins || 0}</td>
                             ...
                             <td className="px-10 py-5 text-right pr-10">
                                <span className="text-2xl font-black text-emerald-500 oswald group-hover:text-yellow-400 transition-colors">{(p.stats?.points || 0).toFixed(1)}</span>
                             </td>
                          </tr>
                       ))}
            ...
            <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
               <div className="bg-zinc-900/50 border border-zinc-800 p-8 rounded-[2rem] flex items-center gap-6">
                  <div className="bg-black p-4 rounded-2xl text-yellow-400"><Star size={24} /></div>
                  <div>
               ...
               <div className="bg-zinc-900/50 border border-zinc-800 p-8 rounded-[2rem] flex items-center gap-6">
                  <div className="bg-black p-4 rounded-2xl text-yellow-400"><Trophy size={24} /></div>
                  <div>
